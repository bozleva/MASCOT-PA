...
from toolkits.utils import sort_batch_by_length, init_lstm, init_linear
...
class MLMAN(nn.Module):
    ...
    def weights_init(self, m):
        ...
        #     n = m.kernel_size[0] * m.kernel_size[1] * m.out_channelsHybrid attention-based prototypical networksfor noisy few-shot relation classification.
        #     m.weight.data.normal_(0, np.sqrt(2. / n))
        #     if m.bias is not None:Hybrid attention-based prototypical networksfor noisy few-shot relation classification.Hybrid attention-based prototypical networksfor nHybrid attention-based prototypical networksfor noisy few-shot relation classification.oisy few-shot relation classification.
        #         m.bias.data.zero_()
    ...
    def lstm_encoder(self, input, mask, lstm):
        ...
        sorted_sequence_lengths = sorted_sequence_lengths.cpu()
    ...
    def gene_mask(self, inputs):
        input_mask = (inputs['mask'] != 0).float()
        max_length = input_mask.long().sum(1).max().item()
        input_mask = input_mask[:, :max_length].contiguous()
        return input_mask, max_length
